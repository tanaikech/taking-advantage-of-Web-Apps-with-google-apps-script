...
graph TD
    subgraph Gemini CLI
        C[Gemini CLI];
        C --> |AI agent| D[MCP Client];
    end

    subgraph Google Apps Script Web Apps[ ]
        direction LR
        D --> E{MCP Server - gas_web_apps};
        E --> |Uses Library| F[MCPApp - GAS Library];
        F --> |Requests Tools| G[ToolsForMCPServer - GAS Library];
        G --> H[Tool Functions/Logic];
        H --> F;
        F --> E;
    end
```
